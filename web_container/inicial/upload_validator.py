...
class FileTypeValidator(object):
    ...
    """
    type_message = _(
        "O tipo de arquivo '%(detected_type)s' não é aceito. "
        "Os tipos aceitos são: '%(allowed_types)s'."
    )

    extension_message = _(
        "A extensão de arquivo '%(extension)s' não é aceita. "
        "Extensões aceitas são: '%(allowed_extensions)s'."
    )

    invalid_message = _(
        "O tipo '%(allowed_type)s' não é um tipo válido. "
        "Veja os válidos em https://www.iana.org/assignments/media-types/media-types.xhtml"
    )
