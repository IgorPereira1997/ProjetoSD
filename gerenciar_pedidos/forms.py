...
class PedidoForm(forms.Form):
    ...
    estoque = forms.IntegerField(required = True,label='Selecione a quantidade',)
...
class TransportadoraPedidoForm(forms.Form):
    ...
    transportadora = forms.ChoiceField(choices=(),required=True,label="Escolha a Transportadora",)

class ModificarPedidoForm(forms.Form):
    def __init__(self, transp, *args, **kwargs):
        super(ModificarPedidoForm, self).__init__(*args, **kwargs)

    statuspedido = forms.ChoiceField(choices=(),required=True,label="Escolha o novo Status",)